    ...
    <>
      ...
      <div className='bg-gray-100 text-gray-800 flex justify-between md:hidden'>
        <div>
          ...
          <Link className='title-font font-medium items-center text-transparent bg-clip-text bg-gradient-to-r from-emerald-500 to-lime-500 mb-4 md:mb-0' to='/'> Dream Car House </Link>
          </div>
        </div>
      ...
      >
        <div>
          ...
          <div className='flex flex-col justify-between flex-1 mt-6'>
            <nav>
              {role && role !== 'requested' ? (
                <>{role === 'admin' ? <AdminMenu /> : <HostMenu role={role}/>} </>
              ) : (
                <UserMenu />
              )}
            </nav>
